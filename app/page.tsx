...
export default function RegexPreview()
{
  ...
  const highlightMatches = ( input: string ) =>
  {
    ...
    if ( lastIndex < input.length ) {
      parts.push( <span key={`e-${ lastIndex }`}>{input.slice( lastIndex )}</span> );
    }
  ...
  const highlightReplaced = ( input: string ) =>
  {
    ...
    for ( ; ; ) {
      ...
      parts.push( <span key={`rb-${ lastPos }`}>{before}</span> );
  ...
  return (
    <div className="w-screen h-screen flex justify-center items-center align-center">
      <div className="w-full h-screen bg-neutral-900 text-neutral-100 flex flex-col items-center py-10 px-4">
        ...
        <div className={cn(
          "w-full max-w-4xl grid gap-6 mt-8",
          ...
          <div>
            ...
            <div className="relative w-full">
              ...
              <textarea
                ...
                className="absolute top-0 left-0 w-full h-64 p-3 rounded-md bg-transparent text-transparent caret-white border border-neutral-700 resize-none overflow-auto font-mono focus:outline-none focus:ring-2 focus:ring-indigo-500"
                style={{ zIndex: 2 }}
                onScroll={( e ) =>
                {
